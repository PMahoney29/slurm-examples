...
ANALYSIS='analysis'      		  # change for every analysis you run (2nd arg)
MAILDOM='@gmail.com'  	      # your email domain (for receiving error messages)
...
PARTITION='short'        		  # the queue on your cluster that allows short jobs
DATADIR="./projects/datain"
RESULTDIR="./result/${ANALYSIS}"  # This is a folder in permanent storage
SCRIPT="./scripts/DugCoy_SSF_clogit_HPC2.R"	# your code as (R or Python) script (1st arg)
...
if [[ -z $RETRYFAILED ]]; then 
    ...
    sbatch --dependency=singleton --job-name=${ANALYSIS} \
           --partition=${PARTITION} --time=0-2 --requeue \
	         --mail-type=FAIL --mail-user="${MAILDOM}" \
    ...
    for i in $(seq 1 $numjobs); do
        ...
        sbatch --dependency=afterok:${waitforjobs} --job-name=${ANALYSIS} \
               ...
               --mail-type=FAIL --mail-user="${MAILDOM}" --time=0-1 \
...
# beginning as we do not yet know which jobs will fail.
    ...
    sbatch --dependency=singleton --job-name=${ANALYSIS} \
           ...
           --mail-type=END,FAIL --mail-user="${MAILDOM}" \
